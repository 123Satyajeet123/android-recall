...
class QueryEngine(private val dao: CaptureDao, private val generate: (String) -> String) {
    ...
    fun query(question: String): String {
        val keyword = Prompts.extractKeyword(question)
        ...
        val searchTerm =
                keyword.split(" ").joinToString(" ") { word ->
                    val stem =
                            when {
        ...
        if (results.isEmpty() && keyword.contains(" ")) {
            ...
            val fallbackStem =
                    when {
                        firstWord.endsWith("ing") && firstWord.length > 6 ->
                                firstWord.removeSuffix("ing")
                        firstWord.endsWith("ed") && firstWord.length > 5 ->
                                firstWord.removeSuffix("ed")
                        firstWord.endsWith("s") &&
                                !firstWord.endsWith("ss") &&
                                firstWord.length > 3 -> firstWord.removeSuffix("s")
                        else -> firstWord
                    }
            val fallbackQuery =
                    SimpleSQLiteQuery(
